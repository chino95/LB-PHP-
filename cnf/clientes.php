...
class Clientes extends ConnectionManager{
	...
	function getClientes(){
		...
		$cnx = $this-> connectMysql();
		try{
			$sth = $cnx->prepare("SELECT e.ID_Cliente, e.Nombre_contacto, e.Nombre_empresa, e.Direccion, e.Telefono, e.Num_Ctpat FROM clientes e
			...
			while($row = $sth->fetch(PDO::FETCH_ASSOC)){
				...
				'<button class="btn btn-embossed btn-primary m-r-20" onclick="MostrarModal('.$row['ID_Cliente'].')">Modificar</button>'));
			}
		}
		catch(PDOException $e){
			$retval['error']=true;
